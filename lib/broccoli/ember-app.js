...
class EmberApp {
  ...
  */
  ...
  _compileAppJsTree(tree) {
    tree = preprocessJs(tree, '/', '/', {
      annotation: `_compileAppJsTree`,
      registry: this.registry,
    });
  ...
  */
  ...
  _addonTree(type, skipTemplates) {
    ...
    let addonTrees = addonBundles.map(addonBundle => {
      ...
      if (experiments.DELAYED_TRANSPILATION) {
        ...
        if (!skipTemplates) {
          precompiledSource = this._compileAddonTemplates(precompiledSource);
        }
    ...
    return combinedAddonTree;
  }

  addonTree() {
    if (!this._cachedAddonTree) {
      let tree = this._addonTree('addon');
  ...
  compiledAddonTree() {
    if (!this._cachedCompiledAddonTree) {
      let tree = this.addonTree();
      ...
      tree = this._compileAddonTree(tree);
      ...
      this._cachedCompiledAddonTree = tree;
    }

    return this._cachedCompiledAddonTree;
  }

  compiledAddonTestSupportTree() {
    if (!this._cachedCompiledAddonTestSupportTree) {
      let tree = this._addonTree('addon-test-support', true);

      tree = this._compileAddonTree(tree, true);
      ...
      this._cachedCompiledAddonTestSupportTree = tree;
    }

    return this._cachedCompiledAddonTestSupportTree;
  }
  ...
  */
  appAndDependencies() {
    ...
    app = this._precompileAppJsTree(app);
    ...
    amdModules = require('broccoli-stew').debug(amdModules, { name: 'amdModules' });
    ...
    if (treeShaking.enabled) {
      strippedAppAndAddons = new Compile([appAndAddons, amdModules], {
        ...
        useNodeModules: treeShaking.useNodeModules,
        annotation: 'Compile',
        missingExportCallback(exportName, importingModule, importedModule) {
          console.log(`${importingModule} cannot find '${exportName}' in ${importedModule}`);
        },
        importCallback: asset => {
          this.import(asset);
        },
      });
    } else {
    ...
    let strippedVendor = new Funnel(strippedAppAndAddons, {
      srcDir: 'vendor',
      destDir: 'vendor',
      allowEmpty: true,
    });
    ...
    let preprocessedApp = this._compileAppJsTree(strippedApp);
    let compiledAddons = this._compileAppJsTree(strippedAddons);
    let compiledNodeModules = this._compileAppJsTree(strippedNodeModules);
    let compiledVendor = this._compileAppJsTree(strippedVendor);
  ...
  */
  styles() {
    if (!this._cachedStylesTree) {
      ...
      let external = this._processedExternalTree();
      let addons = this.compiledAddonTree();
      ...
      let trees = [external, addons].concat(addonTrees, styles);
  ...
  testFiles(coreTestTree) {
    ...
    let addons = this.compiledAddonTree();
    let emberCLITree = this._processedEmberCLITree();
    let addonTestSupportTree = this.compiledAddonTestSupportTree();
    ...
    let baseMergedTree = mergeTrees([emberCLITree, external, addons, coreTestTree, addonTestSupportTree]);
   ...
   */
  otherAssets() {
    let external = mergeTrees([
      this._processedExternalTree(),
      this.compiledAddonTree(),
    ], {
      annotation: 'TreeMerger (otherAssets:external)',
      overwrite: true,
    });
