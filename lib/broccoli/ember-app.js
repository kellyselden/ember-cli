  ...
  */
  ...
  _legacyAddonCompile(type, outputDir, _options) {
    ...
    let addonTrees = addonBundles.map(addonBundle => {
      ...
      tree = this._debugTree(tree, `tree:${type}:${name}`);
      precompiledSource = this._debugTree(precompiledSource, `precompiledSource:${type}:${name}`);
    ...
    if (options.amdFunnelDisabled) {
      ...
      combinedAddonTree = require('broccoli-merge-trees')(addonTrees.concat(compiledSource), {
        overwrite: true,
        annotation: `AmdFunnel TreeMerger (${type})`,
      });
    }

    combinedAddonTree = this._debugTree(combinedAddonTree, `combinedAddonTree:${type}`);
