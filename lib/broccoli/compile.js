...
function _rewriteImportPath(
  ...
  return function rewriteImportPath(id, parent) {
    ...
    if (absolute === path.join(projectRoot, 'config/environment.js')) {
      // workaround for the node_modules resolver
      // finding the project's config
      newPath = relative(projectRoot, newPath);
    } else if (absolute.startsWith(nodeModulesSrc)) {
...
class Compile extends BroccoliPlugin {
  ...
  build() {
    ...
    let importCallback = options.importCallback || function() {};
    ...
    return rollup.rollup({
      ...
      plugins: [
        ...
        {
          transform(code, id) {
            if (id.startsWith('\0commonjs-proxy:')) {
              return '';
            }
            if (id.startsWith(nodeModulesSrc)) {
              let regex = / from '\0commonjs-proxy:(.*)';$/gm;
              let regex2 = /^export { (\w+) as __moduleExports };$/m;
              let matches;
              let newCode = code;
              while (matches = regex.exec(code)) {
                newCode = newCode.replace(`import '${matches[1]}';\n`, '');
              }
              code = newCode;
              code = code.replace(/\0commonjs-proxy:/g, '');
              matches = regex2.exec(code);
              if (matches) {
                // could have false positives in non-cjs transpiled code
                // code = code.replace(
                //   matches[0],
              ...
              // code = code.replace('\0commonjsHelpers', 'vendor/commonjsHelpers');
              if (code.includes('\0commonjsHelpers')) {
                vendorFiles.add('vendor/commonjsHelpers.js');
              }
              return code;
            }
          },
          moduleDest(file) {
            if (file.includes('\0commonjsHelpers')) {
              return 'vendor/commonjsHelpers.js';
            }
            if (file.includes('\0commonjs-proxy:')) {
              file = file.slice(file.indexOf('\0commonjs-proxy:') + '\0commonjs-proxy:'.length);
            }
            let id = path.resolve(appAndAddons, file);
            if (id.startsWith(nodeModulesSrc)) {
              let oldFile = id.substr(nodeModulesSrc.length + 1);
              return path.join(path.relative(destDir, nodeModulesDest), oldFile);
            }
          },
          missingExport(module, name, otherModule, start) {
            missingExportCallback(name, module.id, otherModule.id, start);
            return true;
          },
        },
      ],
      ...
      vendorFiles.forEach(importCallback);
