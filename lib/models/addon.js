...
let addonProto = {
  ...
  */
  treeForSrc(rawSrcTree) {
    ...
    let srcAfterTemplatePreprocessing = this.preprocessTemplates(srcAfterPreprocessTreeHook, {
  ...
  */
  ...
  /**
    Preprocesses a templates tree.

    @private
    @method preprocessTemplates
    @return {Tree} Preprocessed templates
  */
  preprocessTemplates(tree, ...args) {
    let plugin;
    if (experiments.DELAYED_TRANSPILATION) {
      plugin = this.registry.load('template').filter(plugin => plugin.name === 'ember-cli-htmlbars');
      this.registry.remove('template', 'ember-cli-htmlbars');
    }
    if (!experiments.DELAYED_TRANSPILATION || registryHasPreprocessor(this.registry, 'template')) {
      tree = preprocessTemplates(tree, ...args);
    }
    if (experiments.DELAYED_TRANSPILATION) {
      this.registry.add('template', plugin);
    }
    return tree;
  },
  ...
  */
  compileTemplates(addonTree) {
    ...
    if (this.shouldCompileTemplates()) {
      ...
      let processedTemplateTree = this.preprocessTemplates(preprocessedTemplateTree, {
